...
class OrdersList extends Component
{
        ...
        foreach ($this->searchColumns as $column => $value) {
            if (!empty($value)) {
                $orders->when($column == 'order_date', function ($orders) use ($value) {
                        if (!empty($value[0])) {
                            $orders->whereDate('orders.purchase_date', '>=', Carbon::parse($value[0])->format('Y-m-d'));
                        }
                        if (!empty($value[1])) {
                            $orders->whereDate('orders.purchase_date', '<=', Carbon::parse($value[1])->format('Y-m-d'));
                        }
                    })
                    ->when($column == 'username', fn($orders) => $orders->where('users.name', 'LIKE', '%' . $value . '%'))
                    ->when($column == 'subtotal', function ($orders) use ($value) {
                        if (is_numeric($value[0])) {
                            $orders->where('orders.net_amount', '>=', $value[0] * 100);
                        }
                        if (is_numeric($value[1])) {
                            $orders->where('orders.net_amount', '<=', $value[1] * 100);
                        }
                    })
                    ...
                    ->when($column == 'total', function ($orders) use ($value) {
                        if (is_numeric($value[0])) {
                            $orders->where('orders.total_amount', '>=', $value[0] * 100);
                        }
                        if (is_numeric($value[1])) {
                            $orders->where('orders.total_amount', '<=', $value[1] * 100);
                        }
                    });
            }
        }
    ...
    {
        $orders = Orders::whereIn('id', $this->selected)->get();
